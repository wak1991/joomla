...
function saveToLog($file, $text){
    ...
    fwrite($f, getJsDate('now', 'Y-m-d H:i:s')." ".$text."\r\n");
    fclose($f);
return 1;
}
...
function getJsTimestamp($date = 'now', $local = true) {
    return strtotime(getJsDate($date, 'Y-m-d H:i:s', $local));
}
