...
class JshoppingModelUsers extends JshoppingModelBaseadmin{
    ...
    public function publish(array $cid, $flag){
        ...
        $db = JFactory::getDBO();
        $dispatcher = JDispatcher::getInstance();        
        $dispatcher->trigger('onBeforePublishUser', array(&$cid, &$block));
        foreach($cid as $id){
        ...
        $dispatcher->trigger('onAfterPublishUser', array(&$cid, &$block));
    }
}