...
<?php if ( (count($lists['all_independent_attributes'])+count($lists['all_attributes']))>0 ){?>
    <script type="text/javascript">
        ...
        var use_product_ean="<?php print (int)!$jshopConfig->disable_admin['product_ean']?>";
        var use_manufacturer_code="<?php print (int)!$jshopConfig->disable_admin['manufacturer_code']?>";
        var use_product_old_price="<?php print (int)!$jshopConfig->disable_admin['product_old_price']?>";
...
<?php if (count($lists['all_attributes'])){ ?>
    <script type="text/javascript">
       ...
       <?php } ?>
            <th width="100"><?php print _JSHOP_PRICE; ?></th>
            <?php print $this->dep_attr_td_header?>
			<?php if ($jshopConfig->stock){?>            
                <th width="100"><?php print _JSHOP_QUANTITY_PRODUCT ?></th>
            <?php }?>
            <?php if ($jshopConfig->disable_admin['product_ean'] == 0){?>
                <th width="100"><?php print _JSHOP_EAN_PRODUCT ?></th>
            <?php }?>
            <?php if ($jshopConfig->disable_admin['manufacturer_code'] == 0){?>
                <th width="100"><?php print _JSHOP_MANUFACTURER_CODE ?></th>
            <?php }?>
			<?php if ($jshopConfig->admin_show_weight){?>
                <th width="100"><?php print _JSHOP_PRODUCT_WEIGHT?> (<?php print sprintUnitWeight()?>)</th>
			<?php }?>
            <?php if ($jshopConfig->admin_show_product_basic_price){?>
                <th width="100"><?php print _JSHOP_WEIGHT_VOLUME_UNITS ?></th>
            <?php }?>
            <?php if ($jshopConfig->disable_admin['product_old_price'] == 0){?>
                <th width="100"><?php print _JSHOP_OLD_PRICE; ?></th>
            <?php }?>
            <?php if ($jshopConfig->admin_show_product_bay_price){?>
                <th width="100"><?php print _JSHOP_PRODUCT_BUY_PRICE; ?></th>
            <?php }?>
            <th></th>
            <th width="50" class="center"><input type='checkbox' id='ch_attr_delete_all' onclick="selectAllListAttr(this.checked)"></th>
       </tr>
       </thead>
       <?php       
       if (count($lists['attribs'])){
           ...
           foreach($lists['attribs'] as $k=>$v){
               ...
               if ($jshopConfig->disable_admin['manufacturer_code'] == 0){
                print "<td><input type='text' name='attr_manufacturer_code[]' value='".$v->manufacturer_code."'></td>";
               }
       ...
       <div class="col width-55">
        ...
        <legend><?php echo _JSHOP_ADD_ATTRIBUT?></legend>
            ...
            <?php if ($jshopConfig->disable_admin['manufacturer_code'] == 0){?>
            <tr>
                <td class="key"><?php print _JSHOP_MANUFACTURER_CODE?></td>
                <td><input type="text" id="attr_manufacturer_code" style="width:100px;" value="<?php echo $row->manufacturer_code?>"></td>
            </tr>
            <?php }?>
			<?php if ($jshopConfig->admin_show_weight){?>
            <tr>
