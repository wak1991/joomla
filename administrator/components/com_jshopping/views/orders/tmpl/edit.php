...
var admin_show_manufacturer_code=<?php print (int)$this->config->manufacturer_code_in_cart?>;
...
function selectUserBehaviour(uid, eName){
    ...
    updateBillingShippingForUser(uid); 
    SqueezeBox.close();
}
var userinfo_fields = {};
...
<div class="row-fluid">
    ...
    <div class="span10">
        ...
        <a class="modal btn" rel="{handler: 'iframe', size: {x: 800, y: 600}}" href="index.php?option=com_jshopping&controller=users&tmpl=component&select_user=1">
            <?php echo _JSHOP_LOAD?>
        </a>
    </div>
</div>
...
<tr valign="top" id="order_item_row_<?php echo $i?>">
 <td>
   ...
   <a class="modal btn" rel="{handler: 'iframe', size: {x: 900, y: 600}}" href="index.php?option=com_jshopping&controller=product_list_selectable&tmpl=component&e_name=<?php echo $i?>"><?php print _JSHOP_LOAD?></a>
   <br />
   <?php if ($this->config->manufacturer_code_in_cart){?>
   <input type="text" name="manufacturer_code[<?php echo $i?>]" value="<?php echo $item->manufacturer_code?>" title="<?php print _JSHOP_MANUFACTURER_CODE?>"><br />
   <?php }?>
 ...
 <td>
   <input type="text" name="product_quantity[<?php echo $i?>]" class="small3" value="<?php echo $item->product_quantity?>" onkeyup="updateOrderSubtotalValue();"/>   
 </td>
 <td>
...
<tr class="bold">
 <td class="right">
   <?php echo _JSHOP_COUPON_DISCOUNT." ("._JSHOP_CODE.")"?>
 </td>
 <td class="left">
     <input type="text" class="small3" name="coupon_code" value="<?php echo $order->coupon_code?>">
	 <input type="button" class="btn" value="<?php print _JSHOP_CALCULE?>" onclick="order_discount_calculate();">
 </td>
</tr>
...
<tr>
    ...
    <td valign="top">
    	...
    	<div><textarea name="shipping_params"><?php echo $order->shipping_params?></textarea></div>
    </td>
    <?php } ?>
    <?php if (!$this->config->without_payment){?>
    <td valign="top">
